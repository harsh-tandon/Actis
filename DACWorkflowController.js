    ...
    $scope.GetMyDACInfo = function () {
        $http({
            ...
            for (var i = 0; i < result.length; i++) {
                ...
                $scope.txtRemarks = result[i].Remarks.split('~')[0];
            }
        });
...
myapp.controller('AttemptPreIC', function ($scope, $http) {
    ...
    $scope.GetPreICQuestions = function (QgroupID) {
        ...
        $http({
            ...
            //[ITC Infotech - Start] 
            //Check option values are checked. if checked enable next button
            $scope.OptionValuesPrefilled($scope.QuestionsList, $scope.TotalQuestions, $scope.CurrentQuestion)
            //[ITC Infotech - End] 
        });
    }
    ...
    $scope.NextQuestions = function () {
        ...
        $http({
            ...
            //Check option values are checked Yes/No, then call GetFollowupTextForQuestionId
            $scope.OptionValuesPrefilled($scope.QuestionsList, $scope.TotalQuestions, $scope.CurrentQuestion)
        });
    ...
    $scope.GetFollowupTextForQuestionId = function (QuestionId, LinkQuestionId, responseType, followupText) {
        console.log('QID ' + QuestionId + ' Link QID ' + LinkQuestionId + ' Resp Type ' + responseType + ' Followup text ' + followupText)
        $scope.AllifAssetName = localStorage.getItem('AllifAssetName');
        if ($scope.AllifAssetName.toUpperCase() == $("#allifClass").val().toUpperCase()) {
            $scope.ShowFollowupText(QuestionId, responseType, followupText);
            var ItemExists = false;
            $scope.NextIsDisabled = true;
    ...
    //Review page
    $scope.GetFollowupTextForQuestionIdReviewPage = function (QuestionId, responseType, followupText) {
        $scope.AllifAssetName = localStorage.getItem('AllifAssetName');
        if ($scope.AllifAssetName.toUpperCase() == $("#allifClass").val().toUpperCase()) {
            $scope.ShowFollowupText(QuestionId, responseType, followupText);
        }
    ...
    $scope.ShowFollowupText = function (QuestionId, responseType, followupText) {
        console.log(QuestionId + ' ' + responseType + ' ' + followupText)
        if (responseType.toUpperCase() == 'YES') {
            $("#yesFollowUp_" + QuestionId).css("display", "block");
            $("#noFollowUp_" + QuestionId).css("display", "none");
            $("blockquote").remove();
            if (followupText != '' && followupText != null) {
                $("#yesFollowUp_" + QuestionId).append("<blockquote><strong>Follow-up</strong><p id='noFollowUpText_" + QuestionId + "'>" + followupText + "</p></blockquote>");
            }
        }
        else if (responseType.toUpperCase() == 'NO') {
            $("#yesFollowUp_" + QuestionId).css("display", "none");
            $("#noFollowUp_" + QuestionId).css("display", "block");
            $("blockquote").remove();
            if (followupText != '' && followupText != null) {
                $("#noFollowUp_" + QuestionId).append("<blockquote><strong>Follow-up</strong><p id='noFollowUpText_" + QuestionId + "'>" + followupText + "</p></blockquote>");
            }
        }
        else {
            //TODO: N/A
    ...
    //Options are selected, enable next button and increase by if linked questions
    $scope.OptionValuesPrefilled = function (QuestionsList, TotalQuestions, CurrentQuestion) {
        var IsOptionsChecked = false;
        //Check option values are checked. if checked enable next button
        angular.forEach(QuestionsList, function (value, index) {
            var QuestionId = QuestionsList[index].QID;
            var radioBtnCheck = QuestionsList[index].RadioBtnCheck;
            console.log("#yesFollowUp_" + QuestionId);
            if (radioBtnCheck != '' && radioBtnCheck.toUpperCase() == 'NO') {
                IsOptionsChecked = true;
                angular.element('.row').scope().GetFollowupTextForQuestionId(QuestionId, QuestionsList[index].NoLinkQuestionId, 'No', QuestionsList[index].NoFollowupText);
            }
            else if (radioBtnCheck != '' && radioBtnCheck.toUpperCase() == 'YES') {
                IsOptionsChecked = true;
                angular.element('.row').scope().GetFollowupTextForQuestionId(QuestionId, QuestionsList[index].YesLinkQuestionId, 'Yes', QuestionsList[index].YesFollowupText);
            }
        });

        //Disable save button if both questions are same and nothing is selected
        if (TotalQuestions == CurrentQuestion && IsOptionsChecked == false) {
            $scope.isDisabled = true;
        }
    }
    ...
    //        console.log("#yesFollowUp_" + QuestionId);
    //        alert('sss ' + index + ' ' + QuestionList[index].RadioBtnCheck.toUpperCase());
    //        if (QuestionList[index].RadioBtnCheck != '' && QuestionList[index].RadioBtnCheck.toUpperCase() == 'NO') {
    //            //$("#yesFollowUp_" + QuestionId).hide();
    //            //$("#noFollowUp_" + QuestionId).show();
    //            //$("#noFollowUp_" + QuestionId).css('background-color', 'red');
    //            angular.element('.row').scope().GetFollowupTextForQuestionId(QuestionId, QuestionList[index].NoLinkQuestionId, 'No', QuestionList[index].NoFollowupText);
    //        }
    //        else {
    //            angular.element('.row').scope().GetFollowupTextForQuestionId(QuestionId, QuestionList[index].YesLinkQuestionId, 'Yes', QuestionList[index].YesFollowupText);
    //        }
    //    });
    //}

...
myapp.controller('AttemptPreIC', function ($scope, $http) {
    ...
    $scope.GetPreICQuestions = function (QgroupID) {
        ...
        $http({
            ...
            console.log('Question List: ' + $scope.QuestionsList);
            angular.forEach($scope.QuestionsList, function (value, index) {
                ...
                //Allif - add one extra Dummy QuestionGroupId
                if ($scope.AllifAssetName.toUpperCase() == $("#allifClass").val().toUpperCase()) {
                    //if (value.YesLinkQuestionId != null || value.NoLinkQuestionId != null) {
    ...
    $scope.GetFollowupTextForQuestionId = function (QuestionId, LinkQuestionId, responseType) {
        ...
        if ($scope.AllifAssetName.toUpperCase() == $("#allifClass").val().toUpperCase()) {
            if (responseType.toUpperCase() == 'YES') {
                ...
                //Yes, Show
                $("#YesLinkQuestionText_" + QuestionId).show();
                $("#YesLinkQuestionOption_" + QuestionId).show();
                $("#YesLinkComment_" + QuestionId).show();

                //No, Hide 
                $("#NoLinkQuestionText_" + QuestionId).hide();
                $("#NoLinkQuestionOption_" + QuestionId).hide();
                $("#NoLinkComment_" + QuestionId).hide();
            }
                ...
                //Yes, Show
                $("#YesLinkQuestionText_" + QuestionId).hide();
                $("#YesLinkQuestionOption_" + QuestionId).hide();
                $("#YesLinkComment_" + QuestionId).hide();

                //No, Hide 
                $("#NoLinkQuestionText_" + QuestionId).show();
                $("#NoLinkQuestionOption_" + QuestionId).show();
                $("#NoLinkComment_" + QuestionId).show();
            }
            else {
                //TODO: N/A
            }
            ...
            if (LinkQuestionId != null && LinkQuestionId != '' && LinkQuestionId != 0) {
                ...
                if (ItemExists == false) {
                   ...
                   // $scope.TotalQuestions = $scope.TotalQuestions + 1;
                    $scope.LinkedQuestionValueArray.push({ 'LinkQid': LinkQuestionId });
                    if ($scope.isPageIncremented == false && LinkQuestionId != 0 && LinkQuestionId != null && LinkQuestionId != '') {
                        $scope.TotalQuestions = $scope.TotalQuestions + 1;
                        ApplyProgress($scope.TotalQuestions, $scope.CurrentQuestion);
